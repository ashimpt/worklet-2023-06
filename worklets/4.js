...
let f, t0, a0, dn, accent;
let p = 0;
let b2 = 0;
const interval0 = sr / 15;
const interval1 = sr / +5;
...
process(4, function (data, spb, i0, i, t) {
  for (; i0 < spb; i0++, t = ++i / sr) {
    if (i % interval0 == 0) {
      const s = (5 * t) % 10;
      if (rnd(8) < 1) dn = interval0;
      else if ((0b1011110000 >> s) & 1 && (5 * t) % 20 >= 10) dn = interval0;
      else dn = interval1;

      if (i % dn == 0) updateNote(i, t, s);
    }

    p += TAU * f * (1 / sr);
    const a1 = lop(a0);
    const b0 = 0.15 * a1 * sin((2 / 3) * p);
    const a2 = max(accent * 1.5 * asd(5 * t, 0.1), 0.5 * am(3 * t + 0.5));
    b2 = sin(p + b0 + a2 * b2);
